...
Key Features:
    ...
    - FIX: Uses stored metadata answers as RAG context (not raw document chunks)
      This ensures a fair comparison — both Basic RAG and GraphRAG pass
      semantically relevant answer text to the LLM, not bloated document text.
...
class BasicRAGPipeline:
    ...
    Retrieves documents from the 'pubmedqa' ChromaDB collection and uses
    stored metadata answers as context for LLM inference.
    ...
    def retrieve_context(self, query: str) -> str:
        ...
        FIX: Now retrieves stored metadata['answer'] values as context instead
        of raw document chunks (question + context_preview). This produces
        semantically compressed context comparable in spirit to GraphRAG,
        making the token comparison more honest and defensible.

        Args:
            query: User's medical question text.

        Returns:
            str: Concatenated answer snippets from top matching documents.
        """
        try:
            ...
            for meta, dist in zip(metadatas, dists):
                if dist < RELEVANCE_THRESHOLD:
                    # Use the stored answer from metadata as context (compressed, relevant)
                    answer_text = meta.get("answer", "")
                    if answer_text:
                        relevant.append(answer_text)
                else:
